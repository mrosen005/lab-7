...
fish |> 
  ...
  facet_wrap( ~ year) + 
  labs(title = "Frequency of Missing Values Across Years, Sections, and Trips",
...
In the last line of the code, min(length...) has been written when it should be min(weight... ). This exemplifies how easy it can be to make a small mistake in code when duplicating an action multiple times. Writing a function can make this process simpler and can ensure simple mistakes are not made. 
...
fish_long <- fish |> 
  ...
  pivot_longer(cols = c(length,
               ...
               names_to = "Type_of_Length", 
               values_to = "Length") |> 
  mutate(Type_of_Length = fct_recode(Type_of_Length, 
  ...
  geom_histogram() +
  facet_wrap(~ Type_of_Length,
             scales = "free_x") +
  labs(x = "Fish Length", 
       subtitle = "Number of Fish", 
       title = "Original and Rescaled Values of Length", 
       y = "") + 
